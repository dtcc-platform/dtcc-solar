...
from dtcc_solar.scripts.main import analyse_mesh_1
...
class SunpathVisualTest:
    ...
    def setup(self):
        ...
        self.locations = {
            ...
            "Gothenburg": {
                ...
                "GMT_diff": 1,
                "cmap": "autumn_r",
                "file": "data/sunpaths/gothenburg.csv",
            },
            "Stockholm": {
                "name": "Stockholm",
                "lat": 59.33,
                ...
                "GMT_diff": 1,
                "cmap": "autumn_r",
                "file": "data/sunpaths/stockholm.csv",
            },
            "Oslo": {
                "name": "Oslo",
                "lat": 59.91,
            ...
            "Helsinki": {
                ...
                "GMT_diff": 2,
                "cmap": "autumn_r",
                "file": "data/sunpaths/helsinki.csv",
            },
            "Kiruna": {
                "name": "Kiruna",
                "lat": 67.51,
            ...
            "Istanbul": {
                ...
                "GMT_diff": 3,
                "cmap": "autumn_r",
                "file": "data/sunpaths/istanbul.csv",
            },
            "Rio": {
                "name": "Rio de Janeiro",
                "lat": -22.90,
                ...
                "GMT_diff": -3,
                "cmap": "autumn_r",
                "file": "data/sunpaths/rio_de_janeiro.csv",
            },
        }
    ...
    def calc_sunpath_deviation(self, location, plot: bool):
        ...
        deviations = calc_distace_from_sun_pos_to_linear_rings(
            loops, all_sun_pos, gmt_diff
        )
        ...
        for hour in deviations:
            ...
            if relative_error_radius > 0.01 or relative_error_analemma > 0.04:
                print(
                    "Generated sunpath diagram does not match the imported sun paths which have been created using Ladybug in Grasshopper."
                )
                return False

        print(
            "Deviation between generated sunpath and imported sunpath relative to the sun path radius: "
            + str(round(np.average(error) * 100, 4))
            + "% "
            + "for the location "
            + name
            + "."
...
def match_scale_of_imported_sunpath_diagram(
...
def create_sunpath_loops_as_linear_rings(
    loop_pts: Dict[int, List[Vec3]]
) -> List[LinearRing]:
    loops = []
    for key in loop_pts:
...
def calc_distace_from_sun_pos_to_linear_rings(
    loops: List[LinearRing], all_sun_pos: Dict[int, List[Vec3]], gmt_diff: int
) -> Dict[int, List[float]]:
...
def get_distance_between_sun_loops(
    loop_pts: Dict[int, List[Vec3]], radius: float
) -> List[float]:
    avrg_pts = []
    dist = np.zeros(24)
    ...
    for key in loop_pts:
        ...
        avrg_pt = utils.unitize3(avrg_pt)
