...
class SolarEngine:
    ...
    def sun_raycasting(self, suns: List[Sun], results:Results):
        ...
        for sun in suns:
            if sun.over_horizon:
                ...
                irradianceF = mc.compute_irradiance(face_in_sun, face_sun_angles, self.f_count, sun.irradiance_dn)
    ...
    def sky_raycasting(self, suns:List[Sun], results:Results):
        sky_portion = raycasting.raytrace_skydome(self.volume, self.skydome.get_ray_targets(), self.skydome.get_ray_areas())
